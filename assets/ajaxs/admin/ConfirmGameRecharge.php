...
if(isset($_POST['id']))
{
    ...
    if(!$order)
    {
        msg_error("Đơn hàng không tồn tại hoặc đã được xử lý", BASE_URL('admin/confirm-game-recharge'), 2000);
    }

    $user = $CMSNT->get_row("SELECT * FROM `users` WHERE `username` = '".$order['username']."' ");
    if(!$user)
    {
        msg_error("Người dùng không tồn tại", BASE_URL('admin/confirm-game-recharge'), 2000);
    }

    if($user['money'] < $order['money'])
    {
        msg_error("Người dùng không đủ tiền", BASE_URL('admin/confirm-game-recharge'), 2000);
    }
    ...
    msg_success("Xác nhận đơn hàng thành công", BASE_URL('admin/confirm-game-recharge'), 2000);
}
else
{
    msg_error("Thiếu dữ liệu", BASE_URL('admin/confirm-game-recharge'), 2000);
}