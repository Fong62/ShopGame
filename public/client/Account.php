...
if(!isset($_GET['id']) || !isset($_GET['group_slug'])) {
    header("Location: /"); exit();
}

$account_id = check_string($_GET['id']);
$slug = $_GET['group_slug'];
$group = null;

$groups = $CMSNT->get_list("SELECT * FROM `groups`");

foreach ($groups as $grp) {
    if (toSlug($grp['title']) === $slug) {
        $group = $grp;
        break;
    }
}
...
$account = $CMSNT->get_row("SELECT * FROM accounts WHERE id = '$account_id' AND groups = '$group_id'");
if(!$account) {
    header("Location: /"); exit();
}
...
// $og_image = BASE_URL($account['img']);

$GLOBALS['dynamic_meta'] = <<<HTML
<meta name="description" content="Acc game giá rẻ">
<meta property="og:url" content="{og_image}">
<meta property="og:type" content="product">
<meta property="og:title" content="Tài khoản {$group['title']} #{$account['id']}">
<meta property="og:image" content="{$account['img']}">
<meta property="og:image:width" content="1200">
<meta property="og:image:height" content="630">
<meta property="product:price:amount" content="{$account['money']}">
    ...
    <div class="account-container">
        ...
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb" style="background-color: rgba(255,255,255,0.7); border-radius: 5px;">
               ...
               <li class="breadcrumb-item"><a href="/groups/<?=$slug?>"><?=$group['title']?></a></li>
        ...
        <div class="account-details mt-4">
           ...
           <div class="detail-description">
            <?= nl2br(htmlspecialchars($account['chitiet'] ?? 'Không có mô tả')) ?>
        </div>
        </div>
