...
if(!empty($account_id)) {
    ...
    if(!$account_data) {
        msg_error('Tài khoản không tồn tại', BASE_URL('admin/update-accounts'), 2000);
    }
}
...
if($_SERVER['REQUEST_METHOD'] == 'POST') {
    if(isset($_POST['update_account'])) {
        ...
        if($update) {
            msg_success('Cập nhật tài khoản thành công', BASE_URL('admin/update-accounts'), 2000);
        } else {
...
<div class="update-container">
    <div class="update-wrapper">
        ...
        <div class="account-id-input">
            <input type="text" id="account_id_input" class="form-control" placeholder="Nhập ID tài khoản" value="<?=$account_id?>" onkeypress="handleKeyPress(event)">
                <button onclick="redirectToUpdate()" class="btn-select">
        ...
        <table class="account-table">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>
                        <a href="/admin/update-accounts/<?=$account['id']?>" class="btn-select">
...
<script>
    function redirectToUpdate() {
        const id = document.getElementById('account_id_input').value.trim();
        if(id !== '') {
            window.location.href = '/admin/update-accounts/' + id;
        } else {
